...
final class SupabaseAuthService: NSObject, AuthServiceProtocol {
    ...
    func signUp(email: String, password: String, name: String) async throws -> User {
        ...
        do {
            ...
            guard let authUser = session.user else {
                throw AuthError.unknown(NSError(domain: "Auth", code: -1))
            }
    ...
    private func createOrUpdateUserProfile(
        ...
        let profile = UserProfile(
            ...
            displayName: displayName ?? authUser.userMetadata["display_name"]?.stringValue ?? "User",
...
private extension Supabase.User {
    var createdAt: Date {
        // Parse ISO8601 date from Supabase
        ISO8601DateFormatter().date(from: self.createdAt) ?? Date()
    }
}
