...
export function useBibleReader() {
  ...
  const [bookmarks, setBookmarks] = useState<Map<string, VerseBookmark>>(
    new Map(),
  );

  // UI state
  const [selectedVerse, setSelectedVerse] =
    useState<VerseWithAnnotations | null>(null);
  ...
  const loadChapter = useCallback(async () => {
    ...
    try {
      const cached = await getCachedChapter<VerseSource[]>(
        translation,
        currentBook,
        currentChapter,
      );
      ...
      if (!isOffline) {
        try {
          const fresh = await fetchChapter(
            currentBook,
            currentChapter,
            translation,
          );
          ...
          logger.warn(
            "[BibleScreen] Network fetch failed, using cache",
            networkErr,
          );
        }
      }
      ...
      const versesWithAnnotations: VerseWithAnnotations[] = chapterVerses.map(
        (v) => {
  ...
  const handleBookmark = useCallback(() => {
    ...
    if (existingBookmark) {
      ...
      Alert.alert(
        "Bookmark Removed",
        `${selectedVerse.book} ${selectedVerse.chapter}:${selectedVerse.verse} removed from bookmarks.`,
      );
    } else {
      ...
      Alert.alert(
        "Bookmarked!",
        `${selectedVerse.book} ${selectedVerse.chapter}:${selectedVerse.verse} added to your bookmarks.`,
      );
    }
    setSelectedVerse(null);
  ...
  const handleCrossRefNavigate = useCallback(
    (refBook: string, refChapter: number, refVerse: number) => {
  ...
  const handleAIAction = useCallback(
    (action: (typeof AI_QUICK_ACTIONS)[0]) => {
      if (!selectedVerse) return;
