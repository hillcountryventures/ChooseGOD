/**
 * Fill Missing Verses Script
 *
 * Finds and inserts only the missing verses for a given translation.
 */
...
async function insertVerse(verse, retries = 3) {
...
async function getExistingVerseKeys(translation) {
  const keys = new Set();
  let offset = 0;
  const limit = 1000;
  ...
  while (true) {
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/bible_verses?translation=eq.${translation}&select=book,chapter,verse&offset=${offset}&limit=${limit}`,
      {
        headers: {
          'apikey': SUPABASE_SERVICE_KEY,
          'Authorization': `Bearer ${SUPABASE_SERVICE_KEY}`,
...
async function main() {
  const translationCode = process.argv[2];

  if (!translationCode || !TRANSLATIONS[translationCode]) {
    console.log('Usage: node seed-missing-verses.js <translation>');
    console.log('Available translations:', Object.keys(TRANSLATIONS).join(', '));
    process.exit(1);
  }

  const translation = TRANSLATIONS[translationCode];
  console.log(`\n=== Filling Missing Verses for ${translation.name} (${translationCode}) ===\n`);
  ...
  const existingKeys = await getExistingVerseKeys(translationCode);
  ...
  if (missingVerses.length === 0) {
    console.log('All verses already present!');
    return;
  }
  ...
  for (let i = 0; i < missingVerses.length; i += chunkSize) {
    ...
    try {
      ...
      for (let j = 0; j < chunk.length; j++) {
        ...
        try {
          await insertVerse(verseToInsert);
          inserted++;
        } catch (e) {
          if (e.message.includes('duplicate')) {
            inserted++;
          } else {
            console.error(`  Error inserting ${verseToInsert.book} ${verseToInsert.chapter}:${verseToInsert.verse}: ${e.message}`);
          }
        }
      }
  ...
  console.log(`\n\nCompleted: ${inserted} verses inserted for ${translation.name}`);
}
